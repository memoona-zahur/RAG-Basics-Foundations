...
tail_end = len(before_words)            # index in the chunk, not the doc
sentence_start = tail_end
while sentence_start > 0 and not before_words[sentence_start - 1].endswith(SENTENCE_ENDS):
    sentence_start -= 1
head_start = 0
sentence_end = head_start
while sentence_end < len(after_words) and not after_words[sentence_end].endswith(SENTENCE_ENDS):
    sentence_end += 1
severed_sentence_tokens = before_words[sentence_start:] + after_words[: sentence_end + 1]
severed_sentence = " ".join(severed_sentence_tokens)
print(f"The severed sentence:\n    {severed_sentence}")

holds = [
    (i, chunk)
    for i, chunk in enumerate(with_overlap, start=1)
    if sequence_contained(severed_sentence_tokens, chunk)
]
